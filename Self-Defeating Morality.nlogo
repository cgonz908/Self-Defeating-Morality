...
turtles-own [ ptr cooperate-with-same?
  cooperate-with-different?
  trustworthiness       ;; percent of previous interactions that ended in cooperation, starting value is .50 (i.e. the public *perception* of trustworthiness)
  num-cooperate
  num-interactions
]
...
to setup-full
  clear-all
  initialize-variables
  ask patches [ if random-float 1 < chance-to-populate [
                create-turtle ]]
  ask patches [ if-else random-float 1 < chance-to-bloom [
    set pcolor green]
    [set pcolor brown]]
  reset-ticks
end
...
to create-turtle  ;; patch procedure
  sprout 1 [
    ...
    set tot-cooperate tot-cooperate + 1
    set tot-interactions tot-interactions + 2
    set trustworthiness num-cooperate / num-interactions
    set ptr initial-ptr
...
to go
  ...
  ;;If on resource tile, no change
  ask turtles [ ifelse pcolor = green [ set pcolor brown][
    set ptr (ptr - grass-utility)
    ]
  ]
  ask patches [ if pcolor = brown [
    if random-float 1 < chance-to-bloom [
      set pcolor green]
  ]]
  death           ;; kill some of the agents
  update-stats    ;; update the states for the aggregate and last 100 ticks
  if count turtles > 0[
    set avg-trustworthiness mean [num-cooperate / num-interactions] of turtles]
  tick
end
...
to interact  ;; turtle procedure
  ...
  ask turtles-on neighbors4 [
    ...
    ;; Key insight: this function is called *for each turtle that has a neighbor*
    ;; this means that we only have to program the interactions one way because this code
    ;; will be run for the other turtle eventually

    ;; update global and turtle interaction count
    ;; note: receiving ptd doesn't count as an interaction
    set tot-interactions tot-interactions + 1
    ask myself[set num-interactions num-interactions + 1]

    ;;Evaluating the trustworthiness of the other turtle
    ;;trustworthiness (of other turtle) percent chance I determine other turtle is trustworthy
    ifelse random-float 1 < trustworthiness [
      ;;we have determined that the other turtle is trustworthy and we will therefore cooperate
      ask myself [set ptr ptr - cost-of-giving]
      set ptr ptr + gain-of-receiving

      ;;update global and turtle cooperate count
      ask myself [set num-cooperate num-cooperate + 1]
      set tot-cooperate tot-cooperate + 1

    ][ ;; else defect
      set tot-interactions tot-interactions + 1
    ]
      ;;update my trustworthiness
      ask myself [set trustworthiness (num-cooperate / num-interactions)]
  ]
end
...
to death
  ...
  ask turtles [
    if random-float 1.0 < death-rate [
      ;;set tot-cooperate tot-cooperate - num-cooperate
      ;;set tot-interactions tot-interactions - num-interactions
      die ]
  ]
end
...
"" ""
PENS
"Pop. Size" 1.0 0 -10899396 true "" "plotxy ticks count turtles with [shape = \"dot\"]"
...
205
682
Average Trustworthiness Percent
NIL
NIL
0.0
10.0
...
"" ""
PENS
"coop" 1.0 0 -16777216 true "" "plot avg-trustworthiness"
